...
Iteratively go through and collect (?)
"""
...
def findCritsLeft(axis, end, absStart, isMax, distThreshold):
    ...
    while (end != absStart):
        ...
        if (abs(axis[end] - temp) < distThreshold):
            break
...
def findCritsRight(axis, start, absEnd, isMax, distThreshold):
    ...
    while (start != absEnd):
        ...
        if (isMax):
            ...
            isMax = True
            start = nextMax
        if (abs(axis[start] - temp) < distThreshold):
            break
...
def calcDistanceWrapper(xs, ys, x1, y1, x2, y2):
    ...
    for x, y in zip(xs, ys):
        distances.append(calcDistance(x, y, x1, y1, x2, y2))
...
def calcDistance(x, y, x1, y1, x2, y2):
    #issue of handling int times getting overflow and not raising flag...
    #seems to conserve accuracy at this step (but not during csv read)
    x = float(x)
    y = float(y)
    x1 = float(x1)
    y1 = float(y1)
    ...
    #this try/except is for testing float precision
    try:
        numerator = abs((y2-y1)*(x) - (x2-x1)*y + x2*y1 - y2*x1)
        denominator = ((y2-y1)**2+(x2-x1)**2)**(1/2)
    except RuntimeWarning:
        print(x, raw, x1, raw2, x2, raw3)
...
def getVectorsNum(axis, time, curveThreshold = 0.5, distThreshold = 0.5, numVectors = False):
    ...
    if (numVectors):
        ...
        if (currentCount < numVectors):
            ...
            #have many ideas to overcome this (e.g. looking at global max/local max distances for created vectors)
...
"""
IGNORE: used for testing float precision
"""
def getVectorsNum2(axis, time, rawtime, curveThreshold = 0.5, distThreshold = 0.5, numVectors = False):
    #get all the criticals: should contain abs start and abs end of time series
    criticals = getAllCriticals(axis, distThreshold)
    ...
    while stack:
        ...
        # 1. evaluate distance of "real data" to "linear line"
        distances = calcDistanceWrapper(time[index1:index2], axis[index1:index2], rawtime[index1:index2], \
            time[index1], axis[index1], rawtime[index1], time[index2], axis[index2-1], rawtime[index2-1])
        # 2. get max dist point
        maxDistIndex = np.argmax(distances)
