...
# Find minimal ellipsoid via parameters
...
# Check if vector is in ellipsoid
def In_Ellipsoid(point, center, radii, rotation):
    """ This function checks to see if a point is inside an ellipsoid given the
        point in question, and the center, radii, and rotation matrix corresponding
        to the ellipsoid."""
    
    point_from_center = point - center
    rotated_point     = np.matmul(rotation, point_from_center)
    value             = 1
    
    if (rotated_point[0]/radii[0])**2 + (rotated_point[1]/radii[1])**2 + (rotated_point[2]/radii[2])**2 > 1:
        value = 0
